...
[![License: GPL v3](https://img.shields.io/github/license/YanIanZ/KaNeil-Installer)](LICENSE)
...
Unofficial scripts for installing KaNeil Panel & Ship. Works with the latest version of KaNeil!

Read more about [KaNeil](https://kaneil.dev/) here. This script is not associated with the official KaNeil Project.

## Features

- Automatic installation of the KaNeil Panel (dependencies, database, cronjob, nginx).
- Automatic installation of the KaNeil Ship (Docker, systemd).
- Panel: (optional) automatic configuration of Let's Encrypt.
- Panel: (optional) automatic configuration of firewall.
- Uninstallation support for both panel and ship.

## Supported installations

List of supported installation setups for panel and Ship (installations supported by this installation script).

### Supported panel and ship operating systems
...
To use the installation scripts, simply run this command as root. The script will ask you whether you would like to install just the panel, just Ship or both.

```bash
bash <(curl -s https://raw.githubusercontent.com/YanIanZ/KaNeil-Installer/main/install.sh)
```
...
Then you can use `vagrant ssh <name of machine>` to SSH into the box. The project directory will be mounted in `/vagrant` so you can quickly modify the script locally and then test the changes by running the script from `/vagrant/installers/panel.sh` and `/vagrant/installers/ship.sh` respectively.
